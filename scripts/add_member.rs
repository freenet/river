...
//! Add a member to a River room directly via the Freenet API
//!
//! Usage: ./add_member.rs
//!
//! ```cargo
...
use river_core::ROOM_CONTRACT_WASM;
...
async fn main() -> Result<()> {
    ...
    let web_api = WebApi::start(node_url.parse()?).await?;
    ...
    let contract_key = ContractKey::from_params_and_code(Parameters::from(params_bytes), &contract_code);
    ...
    let room_state: ChatRoomStateV1 = match response {
    ...
    let already_member = room_state.members.members.iter().any(|m| m.member.member_vk == github_bot_vk);
    if already_member {
    ...
    web_api.send(ClientRequest::ContractOp(update_request)).await?;

    // Wait for response
    let update_response = tokio::time::timeout(
        std::time::Duration::from_secs(30),
        web_api.recv(),
    ).await??;
