...
impl ContractInterface for WebContainerContract {
    fn validate_state(
        ...
        #[cfg(all(not(test), target_arch = "wasm32", feature = "trace"))]
        {
            ...
            freenet_stdlib::log::info(&format!(
                "Signature (base58): {}",
                bs58::encode(metadata.signature.to_bytes()).into_string()
            ));
            freenet_stdlib::log::info(&format!(
                "Signature length: {} bytes",
                metadata.signature.to_bytes().len()
            ));
            freenet_stdlib::log::info(&format!(
        ...
        if let Err(e) = verify_result {
            #[cfg(all(not(test), target_arch = "wasm32", feature = "trace"))]
            {
                ...
                freenet_stdlib::log::info(&format!(
                    "Expected verifying key (hex): {:02x?}",
                    verifying_key.to_bytes()
                ));
                freenet_stdlib::log::info(&format!("Message length: {} bytes", message.len()));
                freenet_stdlib::log::info(&format!(
                    "First 32 message bytes (hex): {:02x?}",
                    &message[..32.min(message.len())]
                ));
                freenet_stdlib::log::info(&format!(
                    "Last 32 message bytes (hex): {:02x?}",
                    &message[message.len().saturating_sub(32)..]
                ));
            }
            return Err(ContractError::Other(format!(
