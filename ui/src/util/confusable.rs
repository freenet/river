...
//! *reports*. **In this UI both tiers merely show a warning**, so both are
//! used. A tooltip that is occasionally wrong costs nothing, which is exactly
//! why the warning can afford to be fuzzier than any ban ever should be.
//!
...
//!   `generated_handles_do_not_collide_with_each_other` tests are the guard.
...
///
/// Both tiers warn. The distinction is kept because it is exactly what the
/// upstream engine's test table asserts, and because a future change that
/// silently promoted every near-miss to an identical match (or the reverse)
...
pub struct ImpersonationWarning {
    ...
    /// How close the resemblance is. Both tiers render the same warning; this
    /// is retained for tests and diagnostics.
...
impl ImpersonationWarning {
    ...
    ///
    /// Three things, in this order, because a bare ⚠ teaches nobody anything:
    /// what is wrong, that this member is *not* the person they resemble, and
    /// what to look for instead. One definition for every surface, so the
    /// warning cannot say different things in different places — the same rule
    /// [`crate::components::members::DeputyBadge::tooltip`] follows for the
    /// shield.
    pub fn tooltip(&self) -> String {
        match self.impersonated.role {
            ProtectedRole::Owner => format!(
                "Impersonation warning: this name closely resembles the room owner's \
                 (\u{201c}{}\u{201d}), but this member is not the room owner. \
                 The real owner is marked \u{1f451} in the member list.",
                self.impersonated.display_name
            ),
            ProtectedRole::Deputy => format!(
                "Impersonation warning: this name closely resembles the moderator \
                 \u{201c}{}\u{201d}, but this member is not that person. \
                 A real moderator shows a \u{1f6e1} shield next to their name.",
                self.impersonated.display_name
            ),
        }
    }
...
mod tests {
    ...
    /// (`crate::nickname::FIRST_NAMES` x `LAST_NAMES`, 10,000 combinations).
    /// If two of those were confusable, a room where a deputy holds a generated
    /// handle would warn about an innocent member who was simply assigned a
    /// neighbouring one — the exact warning-fatigue failure this feature must
    /// avoid, arriving by default rather than by attack.
    #[test]
    fn generated_handles_do_not_collide_with_each_other() {
        let mut seen: std::collections::HashMap<String, (&str, &str)> =
            std::collections::HashMap::new();
        let mut handles: Vec<(String, &str, &str)> = Vec::new();
        for first in crate::nickname::FIRST_NAMES {
            for last in crate::nickname::LAST_NAMES {
                ...
                handles.push((sk, first, last));
            }
        }
        assert_eq!(handles.len(), 10_000);

        // Near-miss sweep, bounded: a handle can only be within the edit budget
        // of another if their lengths are close, so sort by length and stop
        // comparing once the length gap exceeds the budget.
        handles.sort_by_key(|(sk, _, _)| sk.chars().count());
        let lens: Vec<usize> = handles.iter().map(|(sk, _, _)| sk.chars().count()).collect();
        let chars: Vec<Vec<char>> = handles
            .iter()
            .map(|(sk, _, _)| sk.chars().collect())
            .collect();
        for i in 0..handles.len() {
            let budget = edit_budget(lens[i]);
            if budget == 0 {
                continue;
            }
            for j in (i + 1)..handles.len() {
                if lens[j] > lens[i] + budget {
                    break; // sorted by length: nothing later can be close enough
                }
                if damerau_within(&chars[i], &chars[j], budget) <= budget {
                    panic!(
                        "generated handles {:?} and {:?} are confusable (skeletons {:?} / {:?}); \
                         a deputy holding one would make the other look like an impostor",
                        (handles[i].1, handles[i].2),
                        (handles[j].1, handles[j].2),
                        handles[i].0,
                        handles[j].0,
                    );
                }
    ...
    fn tooltip_names_the_remedy() {
        ...
        assert!(deputy.contains("not that person"), "{deputy}");
        assert!(
        ...
        .tooltip();
        assert!(owner.contains("not the room owner"), "{owner}");
        assert!(
