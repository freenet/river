...
pub fn App() -> Element {
    ...
    #[cfg(not(feature = "no-sync"))]
    {
        ...
        let rooms_signal = use_context::<Signal<Rooms>>().unwrap();
        let status_signal = use_context::<Signal<crate::components::app::freenet_api::SyncStatus>>().unwrap();
        let pending_invites_signal = use_context::<Signal<PendingInvites>>().unwrap();
