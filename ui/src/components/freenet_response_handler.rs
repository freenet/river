//! Handlers for Freenet network responses
//!
//! Contains functions for processing different types of responses from the Freenet network,
//! including state updates, notifications, and status responses.
...
use super::app::freenet_api::SyncStatus;
            ...
            if !was_pending {
                ...
                if let Some(room_data) = rooms_write.map.values_mut().find(|r| r.contract_key == key) {
                    ...
                    if let Err(e) = room_data.room_state.merge(
                        ...
                        *crate::components::app::freenet_api::SYNC_STATUS.write() = SyncStatus::Error(e.clone());
...
pub fn process_ok_response() {
    ...
    *crate::components::app::freenet_api::SYNC_STATUS.write() = SyncStatus::Connected;
