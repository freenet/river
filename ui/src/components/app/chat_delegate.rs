...
pub async fn set_up_chat_delegate() -> Result<(), String> {
    ...
    match api_result {
        Ok(_) => {
            ...
            //
            // (Previously gated under `cfg(not(feature = "example-data"))` to
            // avoid the room-multiplication bug — random owner_vks in
            // create_example_rooms made every cold start look like 3 new
            // rooms to the delegate. That was overcorrection: it also
            // blocked persistence of REAL rooms a user imports while
            // running an example-data build, e.g. via "Import ID". Fixed
            // properly by making `create_example_rooms` use deterministic
            // owner_vks per `example_data.rs` — same keys every launch,
            // HashMap dedups on merge — so the load + save can stay
            // unconditional here.)
