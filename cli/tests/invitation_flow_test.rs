...
fn test_invitation_acceptance_initializes_room_state_correctly() -> Result<()> {
    ...
    assert_ne!(room_state.configuration.configuration.owner_member_id, 
               river_core::room_state::member::MemberId(freenet_scaffold::util::FastHash(0)),
               "owner_member_id should not be default");
    
    assert_eq!(room_state.configuration.configuration.owner_member_id,
               owner_vk.into(),
               "owner_member_id should match the room owner");
    
    assert_eq!(room_state.members.members.len(), 1, 
               "Should have one member (the invitee)");
    
    assert_eq!(room_state.members.members[0].member.member_vk, invitee_vk,
               "Member should be the invitee");
    
    assert_eq!(room_state.member_info.member_info.len(), 2,
               "Should have member info for owner and invitee");
    
    // Verify the room state is valid
    room_state.verify(&room_state, &parameters)
...
fn test_message_validation_after_invitation_acceptance() -> Result<()> {
    use river_core::room_state::message::{MessageV1, AuthorizedMessageV1};
    ...
    room_state.recent_messages.apply_delta(&room_state.clone(), &parameters, &Some(message_delta))
    ...
    assert_eq!(room_state.recent_messages.messages[0].message.content,
               "Hello from invited user!",
...
fn test_uninvited_user_messages_are_filtered() -> Result<()> {
    ...
    assert_eq!(room_state.recent_messages.messages.len(), 0,
